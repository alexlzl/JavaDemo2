...
public class StringUtil {
	...
	public static String removeHtml(String str) {
		return replaceHtml(str).replace("&lt;", "").replace("&gt;", "").replace("&quot;", "").replace("&amp;", "")
				.replace("&nbsp;", "");
	}
	...
	public static boolean isMobileNo(String phone) {
		try {
			if (!StringUtil.isEmpty(phone)) {
				...
				if (phoneNumberText.length() == 11 && "1".equals(firstNumberText)) {
					return true;
				}
			}
		} catch (Exception e) {
	...
	public static boolean isValidEmail(String s) {
		...
		return Pattern.compile("^[_\\.0-9a-zA-Z+-]+@([0-9a-zA-Z]+[0-9a-zA-Z-]*\\.)+[a-zA-Z]{2,4}$").matcher(s).find();
	}
	...
	// String mdkey = StringUtil.md5(HASH_KEY + ch);
	...
	// j = (idx + HASH_CHARS.indexOf(s.charAt(i)) + mdkey.codePointAt(k++)) %
	// 64;
	...
	// public static String decodeRandomHash(String s) {
	// String keys =
	// "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-=+";
	// char ch = s.charAt(0);
	// int idx = HASH_CHARS.indexOf(ch);
	// int indent = idx % 8;
	// String mdkey = StringUtil.md5(HASH_KEY + ch);
	// mdkey = mdkey.substring(indent, indent + indent + 8);
	// s = s.substring(1);
	...
	// for (i = 0; i < cnt; i++) {
	// k = (k == mdkey.length()) ? 0 : k;
	// j = HASH_CHARS.indexOf(s.charAt(i)) - idx - mdkey.codePointAt(k++);
	// while (j < 0) {
	// j += 64;
	// }
	// sb.append(HASH_CHARS.charAt(j));
	...
	public static String getImageUrl(String url, int width, int heigth) {
		if (isEmpty(url)) {
			...
			try {
				...
				String endUrl = url.substring(url.lastIndexOf("."), url.length());
				newUrl = String.format("%s/%dx%d%s", startUrl, width, heigth, endUrl);
			} catch (Exception e) {
				e.printStackTrace();
				LogUtil.e("StringUtil getImageUrl url = " + url + " e = " + e);
				return "";
			}
			return newUrl;
		}
	...
	// return "";
	// }
	// String tempNum = num.substring(num.length()-1, num.length());
	// if(tempNum.equals(".") || tempNum.equals("0"))
	// {
